...
| 문서 | 내용 |
|------|------|
| [테이블설계서](table-spec.md) | ERD, 테이블 컬럼 정의·제약·인덱스, 마이그레이션 이력(V1~V10), 곡명 정규화 규칙 |
| [프로세스 정의서](process-spec.md) | 프로세스 11종 — 일일 파이프라인, 수집, 예측(+변화 요약), RAG 수집·생성, 적중률 평가, RAG Chat, 재생목록, 로그 retention. 흐름도와 오류 격리·외부 API 정책 |
| [인터페이스 정의서](api-spec.md) | REST API 명세(공개·관리자 전체), SSE 계약, RFC 7807 에러 규약, 설계 결정 기록 |
| [화면설계서](screen-spec.md) | 화면 4종(+404) 와이어프레임, 화면 흐름도, 상태 처리 매트릭스, 개선 항목 |
