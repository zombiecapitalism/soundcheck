...
- **예측 근거 분해 (Explainable)** — "왜 이 확률인가"를 등장률 × 최신성 × 유형 부스트로 분해해 표시. 신뢰도 라벨, 셋 내 위치 구간(오프너/본편 3분위/앙코르), 최근 추이(↑/↓)
- **예상 셋리스트 · 예습 코스** — 실제 공연 형태(본편+앙코르 블록)의 예상 셋리스트를 백엔드가 구성. 30분/1시간/2시간 예습 코스(필수/추천/심화 + 규칙 기반 추천 이유)
- **묶음 듣기** — 선택한 곡을 YouTube 임시 재생목록으로 연속 재생 (영상 ID는 검색 후 캐시)
- **AI 공연 어시스턴트 (Chat)** — 예측 통계·배경 문서 두 도구를 근거로만 답하는 채팅(SSE, 레이트리밋)
- **유사 공연 · 장기 통계** — 예측 대상과 가장 비슷한 과거 공연 3건 + 연도/투어별 등장률 차트
- **LLM 계측** — 호출·토큰·캐시 히트·예상 비용을 관리자 AI 대시보드로 상시 관찰
...
| `YOUTUBE_API_KEY` | (선택) 묶음 듣기(재생목록)용 YouTube Data API v3 키 — 비워 두면 해당 기능만 503 |
| `RAG_CONTACT` | (선택) Wikipedia User-Agent에 붙일 연락처 — Wikimedia 정책 권고 |
...
5. **AI 사용량(오늘)** — LLM 호출·평균 지연·캐시 히트율·토큰·예상 비용, 용도별 표
6. **RAG 저장소** — 아티스트별 문서/청크/캐시 현황, 문서 삭제, 설명 캐시 무효화
7. **배치 이력** — collection_log 최근 30건과 진행 상태
